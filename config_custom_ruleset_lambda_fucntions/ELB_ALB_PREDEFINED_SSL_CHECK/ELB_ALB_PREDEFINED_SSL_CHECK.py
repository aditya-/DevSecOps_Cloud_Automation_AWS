...
Description:
  Checks whether your Application Load Balancer TLS/SSL listeners are using a or several predefined policy. The rule is only applicable if there are TLS/SSL listeners for the Application Load Balancer.
...
Rule Parameters:
  ...
  | ---------------------- | ---------- | -------------------------------------------------------------- |
  | ValidPolicies          | Mandatory  | List of TLS/SSL policy name which are valid, separated with comma. |
  | ---------------------- | ---------- | -------------------------------------------------------------- |

Feature:
  In order to: ensure that the traffic is encrypted in transit with the proper method
           As: a Security Officer
       I want: to verify that the configuration of any listener TLS/SSL policy of an application load balancer is correct.
...
Scenarios:
  ...
  Scenario 2:
      ...
      And: At least 1 listener has not their TLS/SSL Policy name listed in the ValidPolicies parameter
     then: Return NON_COMPLIANT

  Scenario 3:
      And: the ALB has at least 1 HTTPS listener
      And: All listeners have their TLS/SSL Policy name listed in the ValidPolicies parameter
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for elb in all_elbv2:
        ...
        https_bool, https_str = is_all_https_listeners_compliant(alb_all_listeners, valid_rule_parameters)
        if not https_bool:
...
def is_all_https_listeners_compliant(listeners, parameters):
    for listener in listeners:
        if 'SslPolicy' in listener.keys():
            if listener['SslPolicy'] not in parameters['ValidPolicies']:
                return False, 'This ALB has a HTTPS listener with a TLS/SSL policy ({}) not listed in the ValidPolicies parameter ({}).'.format(listener['SslPolicy'], ', '.join(parameters['ValidPolicies']))
    return True, None
...
def evaluate_parameters(rule_parameters):
    ...
    if 'ValidPolicies' not in rule_parameters:
        raise ValueError('The parameter "ValidPolicies" must be configured.')
